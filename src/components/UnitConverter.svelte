<script>
  ...
  onMount(() => {
    ...
    if (availableUnits.length > 0) {
      ...
      toUnit = availableUnits.length > 1 ? availableUnits[1].id : availableUnits[0].id;
      calculateConversion();
    }
  });
  ...
  function addToRecentConversions() {
    ...
    recentConversions = [conversionRecord, ...recentConversions.slice(0, 4)];
  }
...
<div class="bg-white rounded-lg shadow-md p-6 max-w-3xl mx-auto">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    <!-- From Unit -->
    <div>
      <label for="fromUnit" class="block text-sm font-medium text-gray-700 mb-2">From</label>
      <select 
        id="fromUnit" 
        bind:value={fromUnit} 
    ...
    <!-- To Unit -->
    <div>
      <label for="toUnit" class="block text-sm font-medium text-gray-700 mb-2">To</label>
      <select 
        id="toUnit" 
        bind:value={toUnit} 
        on:change={calculateConversion}
  ...
  <div class="grid grid-cols-1 md:grid-cols-7 gap-4 items-center mb-6">
    <div class="md:col-span-3">
      <label for="inputValue" class="block text-sm font-medium text-gray-700 mb-2">Value</label>
      <input 
        type="number" 
        id="inputValue" 
    ...
    <div class="md:col-span-3">
      <label for="resultValue" class="block text-sm font-medium text-gray-700 mb-2">Result</label>
      <div 
        id="resultValue"
  ...
  <div class="bg-blue-50 p-4 rounded-md mb-6">
    <h3 class="text-sm font-medium text-blue-800 mb-1">Conversion Formula</h3>
    <p class="text-blue-700">
      1 {availableUnits.find(unit => unit.id === fromUnit)?.name || ''} = 
      {availableUnits.find(unit => unit.id === fromUnit)?.factor && availableUnits.find(unit => unit.id === toUnit)?.factor
        ? formatNumber(availableUnits.find(unit => unit.id === fromUnit).factor / availableUnits.find(unit => unit.id === toUnit).factor)
  ...
  {#if recentConversions.length > 0}
    <div class="mt-8">
      <h3 class="text-lg font-medium text-gray-800 mb-3">Recent Conversions</h3>
      <div class="bg-gray-50 rounded-md overflow-hidden">
        {#each recentConversions as conversion, i}
          <div class="p-3 {i % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-200 last:border-b-0">
            <p class="text-sm text-gray-600">
              {formatNumber(conversion.inputValue)} {conversion.from} = 
              {formatNumber(conversion.result)} {conversion.to}
            </p>
            <p class="text-xs text-gray-500 mt-1">
              {conversion.timestamp.toLocaleTimeString()} - {conversionTypes.find(t => t.id === conversion.type)?.name}
            </p>
          </div>
        {/each}
      </div>
