<script>
    ...
    function computeDiff() {
        ...
        lines1.forEach((line, index) => {
            if (!matchedLines1.has(index)) {
                ...
                let insertPos = 0;
                while (insertPos < diffResult.length && 
                      (diffResult[insertPos].line1 === null || diffResult[insertPos].line1 < index + 1)) {
                    insertPos++;
                }
        ...
        textsAreIdentical = diffResult.every(item => item.type === 'unchanged');
    ...
    function copyDiffResult() {
        const diffText = diffResult.map(item => {
            const prefix = item.type === 'unchanged' ? '  ' : 
                          item.type === 'added' ? '+ ' : '- ';
...
<div class="max-w-7xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Text Differ</h2>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            <label for="text1" class="block text-sm font-medium text-gray-700 mb-2">Original Text</label>
            <textarea 
                id="text1" 
                bind:value={text1} 
        ...
        <div>
            <label for="text2" class="block text-sm font-medium text-gray-700 mb-2">Modified Text</label>
            <textarea 
                id="text2" 
                bind:value={text2} 
                class="w-full h-64 p-3 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
    ...
    {#if diffResult.length > 0}
        {#if textsAreIdentical}
            <div class="mb-4 p-4 bg-blue-100 text-blue-800 rounded-md flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="font-medium">The texts are identical! No differences found.</span>
            </div>
        {:else}
            <div class="border border-gray-300 rounded-md p-4 bg-gray-50 overflow-x-auto">
                <pre class="text-sm font-mono">
                    {#each diffResult as line}
                        <div class="flex">
                            <span class="w-10 text-right text-gray-500 mr-2">{line.line1 || ' '}</span>
                            <span class="w-10 text-right text-gray-500 mr-2">{line.line2 || ' '}</span>
                            <span 
                                class="flex-1 {line.type === 'unchanged' ? 'text-gray-800' : 
